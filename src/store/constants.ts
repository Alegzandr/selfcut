/** How many past states the undo stack retains. */
export const HISTORY_LIMIT = 50;

/** localStorage key for the persisted transport time format preference. */
export const TIME_FORMAT_KEY = 'selfcut.timeFormat';
export const TRACK_HEIGHT_KEY = 'selfcut.trackHeight';

/** localStorage keys for the persisted pane widths (track headers, side panels). */
export const TRACK_HEADER_WIDTH_KEY = 'selfcut.trackHeaderWidth';
export const LIBRARY_WIDTH_KEY = 'selfcut.libraryWidth';
export const INSPECTOR_WIDTH_KEY = 'selfcut.inspectorWidth';

/** localStorage key for the persisted preview resolution preference. */
export const PREVIEW_RESOLUTION_KEY = 'selfcut.previewResolution';

/** localStorage key for the persisted video-scopes mode (waveform / parade …). */
export const SCOPES_MODE_KEY = 'selfcut.scopesMode';

/** localStorage key for the id of the project to reopen on the next startup. */
export const CURRENT_PROJECT_KEY = 'selfcut.currentProjectId';

/** localStorage key for the persisted master preview volume (monitoring only). */
export const PREVIEW_VOLUME_KEY = 'selfcut.previewVolume';

/** localStorage key for the persisted master preview mute (monitoring only). */
export const PREVIEW_MUTED_KEY = 'selfcut.previewMuted';
